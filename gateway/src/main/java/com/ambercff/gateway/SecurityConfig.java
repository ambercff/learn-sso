...
public class SecurityConfig {
    ...
    SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http){
        ...
        http.oauth2Login(conf -> conf
                .authorizationRequestResolver(authorizarionRequestResover(clientRegistrationRepository))
                .authenticationSuccessHandler(new RedirectServerAuthenticationSuccessHandler("http://localhost:3000/profile")));
        return http.build();
    }

    private ServerOAuth2AuthorizationRequestResolver authorizarionRequestResover(ReactiveClientRegistrationRepository clientRegistrationRepository) {
